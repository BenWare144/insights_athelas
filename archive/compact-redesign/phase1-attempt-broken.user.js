    ...
    function applyCompactCss() {
        ...
        const cssChartNote = `
            ...
            */
            ...
            /* ============================================================
               v15 Phase 1: horizontal space reclaim.
               Selectors verified against redesign/dom-clare-progress-note.html
               (line refs noted per rule below).
               ============================================================ */

            /* 1. Hide the global Quasar drawer (250px) and its content-area
                  padding-left. Both are set via INLINE style on the page, so
                  !important is required to win (DOM-FACTS lesson #2).
                  aside.q-drawer          -> dom-clare-progress-note.html:55
                  .q-page-container       -> dom-clare-progress-note.html:56 */
            aside.q-drawer { display: none !important; }
            .q-page-container { padding-left: 0 !important; }

            /* 2. Escape hatch: an invisible 8px hover strip pinned to the left
                  edge (injected by JS, id="ath-nav-peek"). Hovering it reveals
                  the drawer as an overlay (no padding push-back, so content
                  width doesn't jump). html.ath-nav-open (set by the Phase 4
                  toggle) does the same, so the drawer stays reachable once
                  revealed even after the pointer leaves the strip. Note:
                  hovering the drawer's own container isn't wired up here
                  because `aside.q-drawer{display:none}` collapses the
                  container to zero size while hidden, so it can never
                  receive a hover in the first place. */
            #ath-nav-peek {
                position: fixed; top: 0; left: 0; width: 8px; height: 100vh;
                z-index: 9999; background: transparent;
            }
            #ath-nav-peek:hover ~ .q-drawer-container aside.q-drawer,
            html.ath-nav-open .q-drawer-container aside.q-drawer {
                display: block !important;
            }

            /* 3. Sub-nav rail: shrink 160px -> 112px. Keep the v10 rail-
                  compaction rules above (still valid post-rework). Ellipsize
                  item labels instead of shrinking their ExtraSmall font.
                  .tr-w-\[160px\].tr-min-w-\[160px\].tr-max-w-\[160px\]
                  -> dom-clare-progress-note.html:59 */
            .tr-w-\\[160px\\].tr-min-w-\\[160px\\].tr-max-w-\\[160px\\] {
                width: 112px !important;
                min-width: 112px !important;
                max-width: 112px !important;
            }
            .tr-w-\\[160px\\].tr-min-w-\\[160px\\].tr-max-w-\\[160px\\] .MuiTypography-Body\\.ExtraSmall {
                overflow: hidden !important;
                text-overflow: ellipsis !important;
                white-space: nowrap !important;
            }

            /* 4. Content scroller side margins -> 0. Sticky-header row
                  paddings (tr-px-6, several rows) -> 8px, scoped to the sticky
                  header container so nothing outside it is touched.
                  .tr-mx-4.tr-h-full.tr-w-full.tr-overflow-x-auto
                  -> dom-clare-progress-note.html:59
                  tr-px-6 rows inside the sticky header
                  -> dom-clare-progress-note.html:59 (x4) */
            .tr-mx-4.tr-h-full.tr-w-full.tr-overflow-x-auto {
                margin-left: 0 !important;
                margin-right: 0 !important;
            }
            .tr-sticky.tr-top-0.tr-z-10.tr-w-full.tr-border-b.tr-border-Shape-OnSurface-Outlines.tr-bg-Surface-Neutral-Lighter-Surface [class*="tr-px-6"] {
        ...
        // v15 Phase 1: inject the nav-peek hover strip (chart-note only). It
        // must sit as a preceding sibling of .q-drawer-container for the CSS
        // `~` combinator above to reveal the drawer on hover.
        if (isChartNote) {
            const injectPeekStrip = () => {
                if (document.getElementById('ath-nav-peek')) return;
                const drawerContainer = document.querySelector('.q-drawer-container');
                if (!drawerContainer || !drawerContainer.parentNode) return;
                const peek = document.createElement('div');
                peek.id = 'ath-nav-peek';
                drawerContainer.parentNode.insertBefore(peek, drawerContainer);
                log.log('nav-peek hover strip injected');
            };
            if (document.querySelector('.q-drawer-container')) injectPeekStrip();
            else {
                const obs = new MutationObserver(() => {
                    if (document.querySelector('.q-drawer-container')) { injectPeekStrip(); obs.disconnect(); }
                });
